...
const UploadFileForm = ({ loadingAnalysis, onRequestAnalysis }) => {
  const [file, setFile] = useState(null);
  const [type, setType] = useState(null);
  const [options, setOptions] = useState({ fileName: null });
  const [isDragging, setIsDragging] = useState(false);

  const handleDragEnter = () => setIsDragging({ isDragging: true });
  const handleDragLeave = () => setIsDragging({ isDragging: false });
  const onChangeImportFile = (file) => {
    if (file) {
      setFile(file);
      setType(file.type);
      setOptions({ filename: file.name });
    }
  };
  const handleDrop = (e) => {
    e.preventDefault();
